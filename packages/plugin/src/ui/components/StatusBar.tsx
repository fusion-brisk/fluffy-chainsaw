...
const ServiceRow: React.FC<ServiceRowProps> = ({ label, connected }) => (
  <div className="status-bar__popup-row">
    ...
    <span className={`status-bar__dot ${connected ? 'status-bar__dot--ok' : 'status-bar__dot--error'}`} />
    <span className="status-bar__popup-value">
      ...
      {popupOpen && (
        <div ref={popupRef} className="status-bar__popup">
          ...
          {mcpConnected !== undefined && (
            <ServiceRow label="MCP" connected={mcpConnected} />
          )}
