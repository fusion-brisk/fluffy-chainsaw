...
async function copyToClipboard(text: string): Promise<void> {
  try {
    ...
    // Fallback for Figma iframe Permissions-Policy restrictions
...
export const ComponentInspector: React.FC<ComponentInspectorProps> = memo(
  ({ components, onClose }) => {
              ...
              <div key={idx} className="comp-inspector-card">
                ...
                {Object.keys(comp.properties).length > 0 && (
                  <div className="comp-inspector-props">
                    <span className="comp-inspector-props-title">
                      Свойства ({Object.keys(comp.properties).length})
                    </span>
                    <div className="inspector-properties">
                      {Object.entries(comp.properties).map(([propName, propData]) => (
                        <div key={propName} className="inspector-property-row">
                          ...
                          <span
                            className={`inspector-prop-badge inspector-prop-badge--${propData.type.toLowerCase()}`}
                          >
                            {propData.type}
                          </span>
                          <span className="inspector-prop-value" title={String(propData.value)}>
                            {String(propData.value).length > 32
                              ? String(propData.value).slice(0, 32) + '\u2026'
