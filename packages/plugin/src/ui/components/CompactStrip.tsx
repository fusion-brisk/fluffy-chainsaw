...
export const CompactStrip: React.FC<CompactStripProps> = memo(({
  ...
  const menuItems: MenuItem[] = useMemo(() => [
    ...
    // Danger zone
    { id: 'clearQueue', label: 'Очистить очередь', icon: '\u2715', danger: true, condition: !!hasPendingData },
    { id: 'resetSnippets', label: 'Сбросить сниппеты', icon: '\u21BA', danger: true },
  ], [hasPendingData]);

  // Compute menu height for desktop resize
  const menuHeight = useMemo(() => {
    const visibleItems = menuItems.filter(item => item.condition === undefined || item.condition);
  ...
  const closeMenu = useCallback(() => {
    setMenuOpen(false);
    if (platform === 'desktop') {
      onRequestResize(STRIP_HEIGHT);
    } else {
      // Mobile: shrink back from extended
      onRequestResize(STRIP_HEIGHT);
    }
  }, [platform, onRequestResize]);
  ...
  const toggleMenu = useCallback(() => {
    if (menuOpen) {
      ...
      if (platform === 'desktop') {
        ...
        onRequestResize(STRIP_HEIGHT + menuHeight);
  ...
  }, [menuOpen, menuHeight, platform, onRequestResize, closeMenu]);
  ...
  switch (mode) {
    ...
    case 'processing':
      ...
      break;
    case 'success':
      statusIcon = <span className="compact-strip__icon compact-strip__icon--success">{'\u2713'}</span>;
      break;
    case 'error':
      statusIcon = <span className="compact-strip__icon compact-strip__icon--error">{'\u2717'}</span>;
      break;
  }
  ...
  // Progress bar width
  const progressPercent = (mode === 'processing' && current != null && total != null && total > 0)
  ...
  const renderMenuItems = (touchMode: boolean) => {
    ...
    return visibleItems.map(item => {
      ...
      if (item.danger && !renderedDivider) {
        ...
        elements.push(
          <div key="divider" className="compact-strip__menu-separator" />
        );
      }
  ...
  return (
    ...
    >
      ...
      {showTooltip && tooltipText && (
        <div className="compact-strip__tooltip">{tooltipText}</div>
      )}
