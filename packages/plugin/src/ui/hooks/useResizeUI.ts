...
export interface ResizeUI {
  /** Animated resize: state/panel name → size tier → smooth transition */
  resize: (state: string) => void;
  /** Instant resize: cancel any running animation, jump to exact size, update tracking */
 ...
 */
export function useResizeUI(): ResizeUI {
  ...
  const setSize = useCallback(
    (width: number, height: number) => {
      ...
      sendMessageToPlugin({ type: 'resize-ui', width, height });
    },
    [cancelAnimation],
  );
