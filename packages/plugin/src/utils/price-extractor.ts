...
export function extractPrices(container: Element): PriceResult {
  ...
  for (const priceEl of priceElements) {
    ...
    if (
      classes.includes('LabelDiscount') ||
      classes.includes('Discount') ||
      classes.includes('Label_')
    ) {
      continue;
    }
    ...
    if (digits.length >= 3) {
      ...
      const currency = CURRENCY_RUB_REGEX.test(text)
        ? '₽'
        : CURRENCY_USD_REGEX.test(text)
          ? '$'
          : CURRENCY_EUR_REGEX.test(text)
            ? '€'
            : '';
  ...
  if (currentPrices.length > 0) {
    ...
    if (oldPrices.length > 0) {
      ...
      const sameGroupOldPrices = oldPrices.filter(
        (p) => p.priceGroupId === currentPrice.priceGroupId,
      );
      ...
      const sameGroupPrices = sortedCurrent.filter(
        (p) => p.priceGroupId === currentPrice.priceGroupId,
      );
      ...
      if (sameGroupPrices.length > 1) {
        ...
        if (secondPrice.value > currentPrice.value * 1.05) {
          ...
          Logger.debug(
            `⚠️ [extractPrices] Использован fallback: oldPrice=${secondPrice.value} (из того же EPriceGroup)`,
          );
        }
      }
