...
describe('selectFeedVariant', () => {
  ...
  describe('market', () => {
    it('xs size returns variant in 1-2 range', () => {
      const result = selectFeedVariant(makeRow({ '#Feed_CardType': 'market', '#Feed_CardSize': 'xs' }));
    ...
    it('m size returns variant in 3-6 range', () => {
      const result = selectFeedVariant(makeRow({ '#Feed_CardType': 'market', '#Feed_CardSize': 'm' }));
      expect(result).not.toBeNull();
    ...
    it('xl size returns variant in 7-8 range', () => {
      const result = selectFeedVariant(makeRow({ '#Feed_CardType': 'market', '#Feed_CardSize': 'xl' }));
      expect(result).not.toBeNull();
    ...
    it('s size falls into xs range (1-2)', () => {
      const result = selectFeedVariant(makeRow({ '#Feed_CardType': 'market', '#Feed_CardSize': 's' }));
      expect(result).not.toBeNull();
      expect(result!.variant).toBeGreaterThanOrEqual(1);
      expect(result!.variant).toBeLessThanOrEqual(2);
    });
    ...
    it('l size falls into xl range (7-8)', () => {
      const result = selectFeedVariant(makeRow({ '#Feed_CardType': 'market', '#Feed_CardSize': 'l' }));
      expect(result).not.toBeNull();
      expect(result!.variant).toBeGreaterThanOrEqual(7);
      expect(result!.variant).toBeLessThanOrEqual(8);
    });
  ...
  describe('video', () => {
    it('ml size returns variant in 1-5 range', () => {
      const result = selectFeedVariant(makeRow({ '#Feed_CardType': 'video', '#Feed_CardSize': 'ml' }));
      expect(result).not.toBeNull();
      expect(result!.variant).toBeGreaterThanOrEqual(1);
    ...
    it('xs size returns variant in 1-5 range', () => {
      const result = selectFeedVariant(makeRow({ '#Feed_CardType': 'video', '#Feed_CardSize': 'xs' }));
      expect(result).not.toBeNull();
      expect(result!.variant).toBeGreaterThanOrEqual(1);
      expect(result!.variant).toBeLessThanOrEqual(5);
    });
  ...
  describe('post', () => {
    it('with carousel returns variant in 1-4 range', () => {
      const result = selectFeedVariant(makeRow({
        '#Feed_CardType': 'post',
        '#Feed_CardSize': 'm',
